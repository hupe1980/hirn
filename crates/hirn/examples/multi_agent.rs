...
fn main() -> HirnResult<()> {
    ...
    let config = HirnConfig::builder()
        ...
        .build()?;
    let brain = Hirn::open_with_config(config)?;

    // ── Register agents ─────────────────────────────────────────────────
    let alice = AgentId::new("alice").expect("non-empty agent id");
    let bob = AgentId::new("bob").expect("non-empty agent id");

    brain.register_agent(&alice, "Alice — Frontend Engineer")?;
    brain.register_agent(&bob, "Bob — Backend Engineer")?;
    println!("✓ Registered agents: Alice, Bob");

    // ── Agent-scoped contexts ───────────────────────────────────────────
    let ctx_alice = brain.as_agent(&alice)?;
    let ctx_bob = brain.as_agent(&bob)?;
    ...
    let alice_mem = EpisodicRecord::builder()
        ...
        .build()?;
    let alice_id = ctx_alice.remember(alice_mem)?;
    ...
    let shared_id = ctx_alice.remember(shared_mem)?;

    // Both agents can see shared memories
    assert!(ctx_alice.inspect(shared_id).is_ok());
    assert!(ctx_bob.inspect(shared_id).is_ok());
    println!("✓ Shared memories visible to all agents");

    // ── 3. Team namespace ───────────────────────────────────────────────
    brain.create_team_namespace("platform_team", vec![alice.clone(), bob.clone()])?;
    ...
    let team_mem = EpisodicRecord::builder()
        ...
        .build()?;
    let team_id = ctx_bob.remember_in(team_mem, team_ns)?;

    // Both team members can see it
    assert!(ctx_alice.inspect(team_id).is_ok());
    assert!(ctx_bob.inspect(team_id).is_ok());
    ...
    let promoted_id = ctx_alice.share_memory(alice_id, &Namespace::shared())?;
    println!("✓ Alice shared her discovery to shared namespace");

    // Now Bob can see it too
    assert!(ctx_bob.inspect(promoted_id).is_ok());
    ...
    println!("\n── Alice's recall ──");
    let alice_results = ctx_alice.recall(query.clone()).limit(5).execute()?;
    ...
    println!("\n── Bob's recall ──");
    let bob_results = ctx_bob.recall(query).limit(5).execute()?;
    ...
    let alice_knowledge = hirn::semantic::SemanticRecord::builder()
        ...
        .agent_id(alice.clone())
        .build()?;
    brain.store_semantic(alice_knowledge)?;
    ...
    let bob_knowledge = hirn::semantic::SemanticRecord::builder()
        ...
        .agent_id(bob.clone())
        .build()?;
    brain.store_semantic(bob_knowledge)?;

    let result = brain.cross_agent_consolidate(&Namespace::shared(), 0.9)?;
