...
description = "hirn's offline synthesis: budgeted operator jobs plus the background consolidation pipeline — segmentation, RAPTOR, evolution, and forgetting."
weight = 2
+++
...
[agent-tools.md](@/docs/advanced/agent-tools.md)), not by `schedule_offline_job()`, but it shares
...
downstream [`TRACE`](@/docs/hirnql-reference.md) can reconstruct exactly which
...
- [Concepts](@/docs/concepts/_index.md) and [Cognitive Model](@/docs/concepts/cognitive-model.md) — the memory model consolidation feeds
- [HirnQL Reference](@/docs/hirnql-reference.md) — `TRACE`, `NLI`, `ABA`, and the RAPTOR think-mode
- [write-guarantees.md](@/docs/advanced/write-guarantees.md) — how generated cognition is persisted durably
- [architecture.md](@/docs/concepts/architecture.md)
- [security.md](@/docs/security/_index.md)
