+++
title = "Agent Tools"
description = "The 6-function MemoryToolkit that lets LLM agents self-manage cognitive memory over MCP and gRPC, plus the autonomous MemoryAgent maintenance loop."
weight = 4
+++
...
itself follows the [recoverable-envelope contract](@/docs/advanced/write-guarantees.md) so a
...
| `functional_role` | `MemoryType`   | No       | Type-aware composition role (MemGuard): `StableFact`, `EpisodicEvent`, `BehavioralRule`, `Preference` (default: `EpisodicEvent`) |
...
The **functional role** governs type-aware composition at read time (MemGuard,
arXiv:2605.28009): in a conflict, a higher-authority role always wins over a
lower one (`StableFact` > `BehavioralRule` > `EpisodicEvent` > `Preference`),
regardless of recency or source — so a stored *preference* can never override a
*stable fact*. Semantic records default their role from `knowledge_type`.
...
**Multi-entity decomposition (RAG-Fusion).** For comparative ("which did I do
first, A or B?") and duration ("how many days between X and Y?") questions, a
single vector recall tends to surface one side and miss the other. `recall`
transparently decomposes such questions into their compared entities, retrieves a
deep candidate pool for each, and fuses them with the base full-question results
via **weighted Reciprocal Rank Fusion** (RRF, `k=60`; RAG-Fusion) — so both sides'
evidence reaches the result set and documents multiple sub-queries agree on rise.
An entity-coverage fix; a no-op for ordinary single-topic queries. Decomposition
is hybrid: an LLM decomposer (when a provider is configured, robust to paraphrase),
gated behind a cheap comparison-cue pre-check, with a deterministic parser as
offline fallback. On the LongMemEval temporal-reasoning split it lifts recall@10
0.250→0.267 with no change to other categories. The base/entity fusion weights are
tunable via `HIRN_RRF_BASE_WEIGHT` (default: equal weighting).
...
credential's namespace scope. See [Security — Recall is deny-by-default](@/docs/security/_index.md).
...
- The memory model these tools operate on: [Concepts](@/docs/concepts/_index.md) and
  [Cognitive Model](@/docs/concepts/cognitive-model.md).
- The durability each write path promises: [write-guarantees.md](@/docs/advanced/write-guarantees.md).
- The background cognition `schedule_offline_job(...)` drives:
  [offline-intelligence.md](@/docs/advanced/offline-intelligence.md).
- Auditing recall and write decisions: [explanation-surfaces.md](@/docs/advanced/explanation-surfaces.md).
- The query language behind these operators: [HirnQL Reference](@/docs/hirnql-reference.md).
