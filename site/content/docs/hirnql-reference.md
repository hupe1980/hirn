...
description = "Complete reference for HirnQL — the declarative, injection-safe query language for storing, recalling, reasoning over, and administering cognitive memory."
weight = 3
+++
...
For the ideas the verbs operate on — the four memory layers, the property graph, consolidation, and causal reasoning — see [Concepts](@/docs/concepts/_index.md) and the [Cognitive Model](@/docs/concepts/cognitive-model.md). For authorization semantics behind `GRANT`/`REVOKE`, see the [Security Architecture](@/docs/security/_index.md); for running HirnQL against a daemon, see [Deployment & Operations](@/docs/operations/_index.md).
...
Semantic `AS OF OBSERVED` is now **interval-exact**: `SUPERSEDE`, `OVERRIDE`,
`MERGE MEMORY`, and `RETRACT` close the superseded record's `valid_until` at the
successor's `valid_from` (Zep-style `t_invalid`), so a point-in-time query returns
exactly the fact that was valid at the instant — not every prior revision.
`CORRECT` (a transaction-time fix of an always-true fact) deliberately leaves the
observed interval open. Each invalidating change is recorded in the tamper-evident
audit ledger as a `ContradictionResolved` entry (winner/loser revision, relation,
and the closed `valid_until`), so temporal invalidation is independently auditable.

`AS OF` is currently rejected when combined with `EXPAND GRAPH`, `FOLLOW CAUSES`,
or `WITH PROSPECTIVE ON`. Those operators traverse additional present-day graph
or prospective state after seed selection; rejecting the combination prevents a
historical query from mixing snapshot-valid seeds with records that did not yet
exist. Snapshot-aware expansion is planned as an end-to-end operator contract.
