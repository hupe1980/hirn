+++
title = "Architecture"
description = "hirn's crate layout, Lance-backed storage engine, DataFusion query pipeline, property graph, consolidation, Cedar authorization, and language bindings."
weight = 2
+++
...
If you want the shortest task-oriented route through the documentation before diving into internals, start with [Getting Started](@/docs/getting-started.md).
...
[Cognitive Model](@/docs/concepts/cognitive-model.md) first; for causal traversal internals, see
[Causal Reasoning](@/docs/concepts/causal.md); for the write path in depth, see
[Write-Path Intelligence](@/docs/concepts/write-path.md).
{% end %}
...
flowchart TB
  ...
  exec["hirn-exec<br/>19 operators · 2 rules"]
  engine["hirn-engine<br/>HirnDB orchestrator · 11 views"]
  policy["hirn-policy<br/>Cedar 4.11"]
...
- `hirn-core` — types, config, errors, trait definitions (`Embedder`, `TokenCounter`, `Reranker`, `LlmProvider`, `EntityExtractor`, `TextClassifier`, `NliModel`, `EventExtractor`)
...
|-------|------:|------:|---------|
| **hirn-core** | ~7 300 | 208 | `MemoryId` (ULID, `Copy`), `Timestamp`, `Namespace` (interned `Copy`), `AgentId` (interned `Copy`), `Layer`, `EdgeRelation`, `EventType`, `KnowledgeType`, `HirnConfig`, `HirnError`, `Metadata`, `Provenance`, `MemoryContent` (multi-modal), `ResourceObject`, `DerivedArtifact`, `HydrationMode`, `OfflineJobId`, `CognitiveJob`, `OfflineJobTarget`, `OperatorBudget`, and `GeneratedCognitionReview`. `CircuitBreaker`, `AuditAction`/`AuditEntry`, `WelfordStats`, `StringInterner`, `text_util`. **Neuro-symbolic temporal calculus** (`temporal`): `TimeInterval` + Allen's 13 interval relations, duration/gap arithmetic, point-in-time ("as of") containment, and a `Timeline` builder (occurrence-ordered events with per-event Allen relations + gaps + total span + bi-temporal snapshot). **Trait abstractions**: `Embedder`, `AsymmetricEmbedder`, `TokenCounter`, `Reranker`, `LlmProvider`, `EntityExtractor`, `Tokenizer`, `ToolExecutor`. **Natural-language understanding** (`nlu`, not to be confused with the `semantic` memory layer): `ClassificationTask`/`LabelSpec` (the typed decision surfaces every meaning-dependent question runs through), `TextClassifier`, `NliModel`, `EventExtractor`, `Classification`/`DecisionSource`, `Calibration`, `NluBudget` — see [Language Understanding](@/docs/concepts/language-understanding.md). **Record types**: `WorkingMemoryEntry`, `EpisodicRecord`, `SemanticRecord`, `ProceduralRecord`, `SvoEvent`, `ProspectiveImplication`, `PlanningAgenda`, `ReconcileProposal` |
...
| **hirn-exec** | ~10 900 | 146 | DataFusion custom execution layer. **19 physical operators** (each actually emitted into a compiled plan): `LanceHybridSearchExec`, `GraphActivationExec`, `CausalChainExec`, `ContextBudgetExec`, `HebbianBufferExec`, `QueryComplexityExec`, `QualityGateExec`, `IterativeRetrievalExec`, `ContextAssemblyExec`, `RecallMergeExec`, `GlobalSearchExec`, `RaptorSearchExec`, `GraphTraverseExec`, `CausalQueryReadExec`, `TargetedQueryReadExec`, `PolicyQueryReadExec`, `SemanticHistoryScanExec`, `SvoEventScanExec`, and `ProspectiveShortCircuitExec` (via `HirnOp::ProspectiveSearch`). MCFA detection is the shared `mcfa_defense::detect_threat`, enforced on the engine's scored read path with a live `mcfa_audit_log` sink (not a plan operator). Nine never-emitted operators were retired (`NliContradictionExec`, `AbaReconsolidationExec`, `RpeScoreExec`, `ProspectiveIndexingExec`, `SvoExtractionExec`, `InterferenceDetectorExec`, `CausalDiscoveryExec`, `PolicyFilterExec`, `TopicLoomExec`) — the write/consolidation paths are imperative (`db/episodic.rs`, `consolidation/pipeline.rs`), contradiction/reconsolidation run in the imperative admission + consolidation paths, and the ABA decision algorithm now lives in `hirn_engine::resolve_aba`. Ranking uses the single canonical `hirn_core::scoring` composite formula (the former scalar scoring UDFs were deleted). **2 optimizer rules** (`PolicyPushdownRule`, `ActivationFusionRule`; the dead/redundant `TemporalIndexRule`, `NamespacePartitionPruneRule`, and `DepthSchedulingRule` were deleted). `HirnSessionExt` for runtime state injection. `HirnExtensionPlanner` + `HirnQueryPlanner` for LogicalPlan → PhysicalPlan bridging |
| **hirn-policy** | ~1 500 | 22 | Cedar 4.11+ integration, Cedar entity schema, policy store, audit trail, HMAC integrity verification |
...
{% note() %}
See the [Cognitive Model](@/docs/concepts/cognitive-model.md) for the neuroanatomical mapping (dlPFC, hippocampus,
...
**Shared activation core.** The activation math (spreading, static expansion, lateral inhibition, PPR) is implemented once in `hirn_graph::activation_core` as pure functions over per-level adjacency. Both tiers call the same core: the hot tier (`hirn_graph::activation`) materializes adjacency from the in-memory petgraph, the cold tier (`hirn_engine::persistent_activation`) via batched Lance scans (`batch_adjacency_read_*_scoped`, which also push `namespace IN (...)` into the scan). Hot and cold therefore have identical semantics — same decay/epsilon math, same per-level `max_frontier_size` safety cap, same Jaccard-modulated inhibition, same forward+backward PPR reachability — and deterministic ordering: frontiers and result orderings sort by score descending with `MemoryId` as a stable secondary key, so repeated runs (and both tiers) produce identical orderings and traces.
...
The PPR subgraph is the **forward + backward** reachable set from the seeds — outgoing edges pull in downstream effects, incoming edges pull in upstream causes (seeds should surface what caused them, not only what they cause). The transition matrix itself stays out-edge-only. Both tiers use the same rule: the hot tier walks petgraph in both directions, the cold tier pairs `batch_adjacency_read_scoped` (by `source`) with `batch_incoming_adjacency_read_scoped` (by `target`).
...
validation, and Bayesian accumulation are roadmap. See [Causal Reasoning](@/docs/concepts/causal.md#causal-discovery-during-consolidation).
{% end %}
...
hirn uses [Cedar](https://www.cedarpolicy.com/) (`cedar-policy` v4.11.2, Apache 2.0, CNCF project) for fine-grained authorization. Cedar replaces hand-rolled ACL types with a formally verifiable policy language supporting RBAC, ABAC, entity hierarchies, and automated reasoning.
...
Pearl's 3-rung causal hierarchy. See [causal.md](@/docs/concepts/causal.md) for full documentation.
