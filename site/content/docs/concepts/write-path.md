...
description = "What hirn does at write time — RPE-gated admission, prospective indexing, SVO extraction, interference tracking, and multimodal resource routing."
weight = 4
+++
...
(`false`), every write takes the slow path. See the [Cognitive Model](@/docs/concepts/cognitive-model.md#rpe-reward-prediction-error-the-admission-gate)
...
Two extractors, selected by configuration — see
[Language Understanding](@/docs/concepts/language-understanding.md):

- **Typed extraction** (`nlu.typed_event_extraction = true`, requires an LLM
  provider): schema-constrained extraction that resolves passive voice to the
  real actor ("the release was deployed by Alice" → subject `Alice`, not `the
  release`) and marks negated assertions.
- **Regex extraction** (always available): surface SVO patterns. It has no clause
  scope, so it reads the first capitalized word as the subject and cannot
  represent negation at all.

A typed extractor that returns nothing — timeout, malformed output, or a
genuinely event-free record — falls through to the regex extractor rather than
dropping the record's events.

Events are stored in the `svo_events` dataset with:

- `source_memory_id` FK
- Subject, verb, object text
- `time_start` and `time_end` (BTree-indexed)
- Confidence score
- `negated` — whether the source asserts the event did **not** happen

The `negated` column matters more than it looks: "we never shipped v2" and "we
shipped v2" produce the same subject, verb, and object. Without the flag they are
the same stored event, and a timeline query answers that v2 shipped. Regex-extracted
events are always `negated = false` because surface patterns cannot establish it.
...
[nlu]
typed_event_extraction = false   # true = model extraction, one call per record
```
