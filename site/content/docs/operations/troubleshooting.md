...
description = "Diagnose operator-facing hirn failures — config errors, HTTP status mapping, rate limits, Cedar denials, provider partial failures, and offline job stalls."
weight = 5
+++
...
If you are not sure which guide you should be using yet, start with [Getting Started](@/docs/getting-started.md).
...
The MCP surface mirrors the same distinction, so a refusal reads the same whichever interface a client uses:

| JSON-RPC Code | Error Shape | Typical Cause |
|---------------|-------------|---------------|
| `-32602` (invalid params) | `access denied: ...` | Cedar denial, or a namespace the caller cannot reach |
| `-32602` (invalid params) | `not found: ...` | missing memory or resource |
| `-32603` (internal error) | `internal server error` | everything else, redacted |

**A refusal is not a fault.** Reading a record in another agent's private namespace is
denied by design — every record lands in its author's private namespace, and
cross-agent private reads are refused. If you see `access denied: agent 'x' cannot
access namespace 'private:y'`, the caller identity and the record's author differ; that
is the isolation boundary working, not a bug. Write and read as the same identity, or
put the record in `shared`.

Errors outside those two variants stay redacted on purpose. `InvalidInput` in
particular looks caller-facing but is also what a non-transient provider failure
converts into, carrying provider status and response body — so it is not surfaced. When
a client sees `internal server error`, the detail is in the daemon's `tracing` output
under the `MCP request failed internally` event.
...
header on its own is never trusted. See [Security](@/docs/security/_index.md).
{% end %}
...
- [docs/cedar-guide.md](@/docs/security/cedar-guide.md)
- [docs/cedar-patterns.md](@/docs/security/cedar-patterns.md)
...
1. Check `hirn_offline_job_queue_depth`, `hirn_offline_job_running`, `hirn_offline_job_failed_total`, and `hirn_offline_job_skipped_total` in [observability.md](@/docs/operations/observability.md).
...
If review or rollback behavior is the surprise, read [offline-intelligence.md](@/docs/advanced/offline-intelligence.md) and [security.md](@/docs/security/_index.md) together. The behavior is intentionally conservative.
...
- [docs/performance-tuning.md](@/docs/operations/performance-tuning.md)
