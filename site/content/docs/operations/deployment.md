+++
title = "Deployment"
description = "Deploy hirn embedded, as a standalone hirnd daemon, or as a multi-node Raft cluster — with the CLI flags, ports, and TLS/auth model for each."
weight = 1
+++
...
See [Security](@/docs/security/_index.md) for the full authentication and policy model.
...
2. **Offline cognition jobs** — one `dream`, one `reconcile`, and one
   ...
   budget (see [Offline Intelligence](@/docs/advanced/offline-intelligence.md)). The scheduler
...
Raft state-machine metadata (realm ownership, the node registry, active leases, and the
monotonic lease fence) is **persisted durably** to the same embedded `redb` database as
the Raft log (under `raft.data_dir`) and reloaded on restart. It survives openraft's
snapshot-driven log compaction — the state machine is seeded from the durable snapshot +
applied-state rows, not rebuilt from a purged log. The lease fence is strictly monotonic
across restarts (it never regresses). Only `insecure_dev_mode` without a `data_dir` uses a
volatile in-memory store, which loses this metadata on restart and must not be used in
production.
...
The [sleep-time consolidation](#sleep-time-consolidation) pass
...
- **Duration:** 5 minutes, renewed every TTL/3 while consolidation runs and once between
  pass phases, then released when the pass finishes. A rejected renewal cancels the realm
  pass. If the holder crashes, the lease expires and another node picks the realm
