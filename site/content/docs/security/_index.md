+++
title = "Security"
description = "hirn's defense-in-depth security model: Cedar authorization, MCFA prompt-injection defense, namespace isolation, and a tamper-evident HMAC audit trail."
weight = 5
sort_by = "weight"
...
embedded [`hirn::Memory`](https://github.com/hupe1980/hirn/blob/main/README.md) facade and in-process admin tooling that own
...
token-restricted identity intersects the JWT namespace allowlist with its agent's
engine/Cedar/team grants (via `db.as_agent_with_namespaces(...)`); a credential can
therefore narrow but never widen access. By-ID inspect, archive, purge, link, and
HirnQL paths use the same narrowed context. An unrestricted API-key/mTLS identity — which owns every
...
## Write-Path Poisoning Defense (A-MemGuard)

When `admission_poisoning_action = Quarantine`, the admission poisoning gate computes
a deterministic, LLM-free combined **poison score** for every candidate write:

| Signal | Weight | What it detects |
|--------|--------|-----------------|
| Content injection | 0.30 | `detect_injection` patterns (system-override, chat-template tokens, injection phrases) over a homoglyph-resistant skeleton |
| Trusted-fact contradiction | 0.30 | High similarity to a high-confidence, high-authority in-namespace `StableFact` **combined with** negation/override markers |
| Embedding outlier | 0.15 | Anomalous dissimilarity to the corpus (+ future-dated timestamp) |
| Low trust | 0.15 | `1 − effective_trust` (provenance origin × agent reputation) |
| Authority mismatch | 0.10 | Candidate's `functional_role` authority vs the nearest trusted record |

To avoid blocking legitimate novel writes, quarantine requires **at least two
corroborating strong signals** and is skipped during cold start (< ~10 records). The
score maps to three bands: below the quarantine threshold → **admit** (optionally
flagged); `[quarantine, reject)` → **quarantine for review** (deferred trust — the
record is held out of the live store, not rejected); at or above the reject threshold
→ **reject**. Quarantine routing works for both episodic and semantic writes and flows
through the existing quarantine dataset, the hash-chained audit ledger
(`AuditAction::Quarantine` carrying the poison score), and the Cedar `review` action
for release. Thresholds are `admission_poisoning_quarantine_threshold` (default 0.5)
and `admission_poisoning_reject_threshold` (default 0.85). Stored content is never
rewritten. This complements the read-path MCFA defense below (defense in depth:
poisoned content is caught at admission *and* filtered at read).
...
An admission `Defer` is an explicit **manual-review** outcome, not an automatic
retry queue. The quarantined record carries `admission_review_not_before_ms`;
the Cedar-gated review path fails closed before that time and may approve or
reject it afterward. No background worker promotes deferred content.
...
Encoding or append failure is request-fatal after bounded retries. Flagged rows
remain excluded, but hirn does not return a successful read while silently losing
the corresponding security evidence.

## Provider Transport Security

Remote provider traffic is HTTPS-only; plaintext HTTP is accepted only for explicit
loopback development endpoints. The shared provider client disables redirects and
environment proxies, validates literal IPv4/IPv6 addresses, and rejects DNS answers
containing private, loopback, link-local, multicast, unspecified, carrier-grade NAT,
or benchmark-range addresses. This closes redirect and DNS-rebinding SSRF paths.
JSON responses are capped at 16 MiB and error bodies at 64 KiB. Embedding and LLM
retry wrappers use bounded jittered backoff and retain typed rate-limit delays such
as `Retry-After`.
...
[Deployment & Operations](@/docs/operations/_index.md) for the transport auth model.
{% end %}
...
verified. Engine startup emits a warning in this mode; production deployments
should treat `event_hmac_secret` as required and keep it outside the brain store.
...
[Encryption at Rest](@/docs/security/encryption-at-rest.md).
{% end %}
