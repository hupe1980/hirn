+++
title = "Cedar Guide"
description = "Write authorization policies for hirn with Cedar: the entity model, 20 actions, RBAC and ABAC, realm isolation, and runtime policy management via HirnQL."
weight = 1
+++
...
hirn uses [Cedar](https://www.cedarpolicy.com/) (`cedar-policy` v4.11.2) for fine-grained authorization. Cedar is an open-source policy language created by AWS and donated to the CNCF. It supports RBAC, ABAC, entity hierarchies, schema validation, and automated reasoning.
...
The policy crate exposes `PolicyEngine::authorize_with_context(request, json)` for
schema-validated Cedar context attributes. Invalid, missing, or mistyped attributes
fail closed. Embedded integrations may use this API for trusted facts such as request
time, client network, or MFA state after declaring those attributes in the Cedar
schema.

The current daemon authorizes from authenticated realm, agent, operation, and
namespace scope; it does **not** synthesize arbitrary IP/time/MFA Cedar context.
Do not deploy a context-dependent policy on HTTP/gRPC/MCP until the trusted ingress
attribute and schema mapping are explicitly wired and tested. JWT namespace and
operation claims are enforced separately and intersected with Cedar/team grants.
...
For a larger operator-oriented pattern catalog, see [docs/cedar-patterns.md](@/docs/security/cedar-patterns.md).
...
- [hirn Architecture — Cedar Authorization](@/docs/concepts/architecture.md#cedar-authorization-audit-trail)
